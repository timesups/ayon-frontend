...
const UserAccessForm = ({ accessGroupsData, formData, onChange, disabled}) => {
  ...
  const getTooltip = () => {
    if (isUser) {
      return t("Users cannot be developers")
    }
    if (isManager) {
      return t("Managers cannot be developers")
    }
    if (authenticatedUser.isManager) {
      return t("Only admins can set developers")
    }

    return t("Developers have access to enhanced tools and features")
  }

  return (
    <>
      <b>{t("Access Control")}</b>
      <FormLayout>
